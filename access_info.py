...
def generation_floor_device_name(project):
    ...
    for entry in generation_device_info_dict(project):
        d_xoa_name = ['-'.join((device_name_prefix.device_prefix(mysql_table_query.workplace_info(project)[0]['city'],mysql_table_query.workplace_info(project)[0]['building_name']),(str(entry['floor'])+str(entry['bdr']).rjust(2,'0')),'D',entry['xoa'],str(num).rjust(2,'0'))) for num in range (1,entry['d_xoa']+1)]
...
def get_access_info(project):
    ...
    for n,m in  zip(generation_floor_device_name(project),access_manage_ip(project)):
        ...
        for name in n['DXOA']:
            ...
            dxoa = {'floor':n['floor'],'name':name,'ip':str(ip_address[0]),'netmask':str(ip_address[1]),'port_assign':device_port.h3c(type(project)['xoa'])}
        ...
        for name in n['EXOA']:
            ...
            exoa = {'floor': n['floor'], 'name': name, 'ip': str(ip_address[0]),
                    'netmask': str(ip_address[1]), 'port_assign': device_port.h3c(type(project)['xoa'])}
