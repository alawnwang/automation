...
def generation_ccs_name(project):
    ccs_prefix = '-'.join((device_name_prefix.device_prefix(mysql_table_query.workplace_info(project)[0]['city'],mysql_table_query.workplace_info(project)[0]['building_name']),'BDR'+str(mysql_table_query.workplace_info(project)[0]['core_floor'])+'01'))
    return ccs_prefix
...
def get_ccs_fw_info(network,project):
    ...
    mgt_dict = {'floor': entry['floor'],'bdr':entry['bdr'],'up_phycial':str(ccs_fw_connect_ip[4])+'/'+str(ccs_fw_connect_ip.prefixlen),'down_phycial':str(ccs_connect_ip[3])+'/'+str(ccs_connect_ip.prefixlen),'MCW': ('-'.join((generation_ccs_name(project), 'A01', str(get_ccs_fw_type(project)['name']), 'CCS', '01')))
        , 'MMGTIP': {'up_link_manage_ip': str(ccs_fw_connect_ip[5])+'/'+str(ccs_connect_ip.prefixlen),'down_link_manage_ip':str(ccs_connect_ip[1])+'/'+str(ccs_connect_ip.prefixlen)}, 'SCW': (
            '-'.join((generation_ccs_name(project), 'A02', get_ccs_fw_type(project)['name'], 'CCS', '01'))),
                'SMGTIP': {'up_link_manage_ip': str(ccs_fw_connect_ip[6])+'/'+str(ccs_connect_ip.prefixlen),'down_link_manage_ip':str(ccs_connect_ip[2])+'/'+str(ccs_connect_ip.prefixlen)},'port_assign': get_ccs_fw_type(project)['port_assign']}
